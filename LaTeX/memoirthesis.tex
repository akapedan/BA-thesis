...
\documentclass[a4paper,11pt,leqno,openbib,11pt,openany]{memoir} %add 'draft' to turn draft option on (see below)
%
\linespread{1.2}
% Adding metadata:
\usepackage{datetime}
...
\settypeblocksize{648pt}{448.13pt}{*} 
\setulmargins{3.0cm}{*}{*} 
\setlrmargins{*}{*}{1.5} 
...
% enables double spacing but leaves single spaced for captions for instance. \OnehalfSpacing 
%
...
%\usepackage{subfig}                 %Two pictures side-by-side
...
\usepackage{siunitx}       %Center table values
\newcolumntype{L}{>{\phantom{$\mathbin{-}$}$}l<{$}}
\newcommand{\mi}{\mathllap{\mathbin{-}}}

\usepackage{amsfonts} 					%Calls Amer. Math. Soc. (AMS) fonts
\usepackage[centertags,leqno]{amsmath}  %Writes maths centred down
\makeatletter
...
\usepackage[applemac]{inputenc}			%Needed to encode non-english characters 
									%directly for mac
\usepackage{xcolor,colortbl}					%Makes coloured tables
\usepackage{threeparttable}             %Footnote after table
...
\usepackage[authoryear,square,semicolon,sort&compress]{natbib} %Calls bibliography commands 
...
\backmatter
%\bibliographystyle{munich.bst} %german
...
%\bibliographystyle{computational-complexity/cc.bst}
%\bibliographystyle{gost780.bst}
\bibliographystyle{biblio2.bst}
%\bibliographystyle{biochem.bst}


\refstepcounter{chapter}
\bibliography{shkenca}
